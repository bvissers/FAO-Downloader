...
class WaPORwidget(qtw.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ui.ButtonDownload.clicked.connect(lambda: self.StartDownload())
...
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    def StartDownload(self):
        #self.thread_manager.start(self.CheckInputs)
        self.th = threading.Thread(target=self.CheckInputs)
        self.th.start()
    ...
    def CheckInputs(self):
        x = 0
        self.ui.labelStatus.setText("Status: Checking Inputs")
        ...
        self.ave_cum = self.ui.comboDekadal.currentText()
        if x == 0:
            self.CancelDownload = False
    ...
    def DownloadRequest(self,latlim, lonlim):        
            ...
            self.ui.labelStatus.setText("Status: Preparing Download")
            m=0
            self.query_accessToken()
            for  cube_code in self.SelectedCubeCodes:
                if self.CancelDownload == False:
                    m+=1
                    self.ui.labelProgress.setText("Progress: Starting download for Wapor data {0}.\n Item number {1} of {2}".format(cube_code, m,  str(len(self.SelectedCubeCodes))))
                    print("Progress: Starting download for Wapor data {0}.\n Item number {1} of {2}".format(cube_code, m,  str(len(self.SelectedCubeCodes))))
                    self.ui.labelStatus.setText("Status: Creating Data list")
                    thread = threading.Thread(target=lambda:self.Get_df(cube_code, self.StartDateEdit, self.EndDateEdit) )
                    thread.start()
                    thread.join()
                   ...
                   #self.ui.labelStatus.setText("Status: Constructing WaPOR Request")
                    if 'lcc' in cube_code.lower():
                        ...
                        pass
                    n=0
                    for index, row in self.df_avail.iterrows():
                        if self.CancelDownload == False:
                            n+=1            
                            self.ui.labelProgress.setText("Progress: Starting download for Wapor data {0}. \nItem number {1} of {2} \nDownloading raster {3} of {4}".format(cube_code, m,  str(len(self.SelectedCubeCodes)), n,str(len(self.df_avail))))
                            
                            thread2 = threading.Thread(target=lambda:self.WaporRequest(cube_code, bbox=bbox, row=row ) )
                            thread2.start()
                            thread2.join()
                            ...
                            thread = threading.Thread(target = lambda:self.Tiff_Edit_Save(cube_code,  multiplier, row, savefolder, resp) )
                            thread.start()
                            thread.join()
            ...
            if self.CancelDownload == False:
                ...
                self.ui.labelStatus.setText("Status: Download Canceled")
 
    def Get_df(self, cube_code, Startdate, Enddate):
        time_range='{0},{1}'.format(Startdate,Enddate)
        try:
            self.df_avail=self.getAvailData(cube_code,time_range)
            return
        except:
    ...
    def Tiff_Edit_Save(self, cube_code,  multiplier, row, savefolder, resp):                  
      #check this works for seasonal and non seasonal
              try:   
                  ...
                  if self.cubedict[cube_code]['cubedimensions']['code']=='DEKAD' and self.ave_cum == 'Cumulative' :
                  ...
                  print("Status: Correcting raster")
    ...
    def getCropRasterURL(self,bbox,cube_code,
        ...
        print("Status: Readying Download")   
        #Create Polygon        
    ...
    def _query_jobOutput(self,job_url):
     ...
     while contiue:        
         ...
         print("Status: Waiting for WaPOR to finish request")
